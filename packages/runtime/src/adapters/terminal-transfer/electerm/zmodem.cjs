/* eslint-disable @typescript-eslint/no-require-imports, @typescript-eslint/no-unused-vars, no-undef, no-empty */
...
class ZmodemSession {
  ...
  prepareReceiveFile (name, size) {
    try {
      ...
      const stream = fs.createWriteStream(this.downloadTempPath, {
      ...
      stream.on('error', (e) => {
        ...
        this.sendToClient({
          event: 'transfer-error',
          message: 'Failed to write received file'
        })
  ...
  handleFileComplete () {
    ...
    const finalize = () => {
      try {
        if (filePath && temporaryPath) fs.renameSync(temporaryPath, filePath)
        if (filePath && fileMtime > 0) {
          const mtimeDate = new Date(fileMtime)
          fs.utimesSync(filePath, mtimeDate, mtimeDate)
        }
        this.sendToClient({ event: 'file-complete', name: currentTransfer?.name })
      } catch (e) {
        if (temporaryPath) try { fs.unlinkSync(temporaryPath) } catch {}
        this.sendToClient({ event: 'transfer-error' })
      }
    }

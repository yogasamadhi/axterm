...
// Adapted from Electerm 5.5.0; copyright ZHAO Xudong. See UPSTREAM.md.
/**
...
const log = require('./safe-log.cjs')
const generate = require('./uid.cjs')
const sanitizeFilename = require('./sanitize-filename.cjs')
...
class XmodemSession {
  ...
  handleData (data) {
    ...
    if (this.state === XMODEM_STATE.WAITING_SAVE_PATH) {
      if (this.receiveBuffer.length + data.length > MAX_BUFFERED_BYTES) {
        this.sendToClient({ event: 'session-error' })
        this.endSession()
        return true
      }
    ...
    if (this.state === XMODEM_STATE.WAITING_FILES) {
      if (this.pendingSendBytes + data.length > MAX_BUFFERED_BYTES) {
        this.sendToClient({ event: 'session-error' })
        this.endSession()
        return true
      }
  ...
  handleReceiveData (data) {
    this.clearReceiveTimeout()
    this.receiveBuffer = Buffer.concat([this.receiveBuffer, data])
    if (this.receiveBuffer.length > MAX_BUFFERED_BYTES) {
      this.sendToClient({ event: 'session-error' })
      this.endSession()
      return
    }
  ...
  prepareReceiveFile () {
    ...
    this.downloadTempPath = `${filePath}.axterm-${generate()}.part`
    this.downloadStream = fs.createWriteStream(this.downloadTempPath, {
  ...
  handleReceiveComplete () {
    ...
    const temporaryPath = this.downloadTempPath
    ...
    const finish = () => {
      try {
        if (temporaryPath && filePath) fs.renameSync(temporaryPath, filePath)
        this.sendToClient({ event: 'file-complete', name: fileName })
      } catch {
  ...
  endSession () {
    ...
    if (this.downloadTempPath) {
      try { fs.unlinkSync(this.downloadTempPath) } catch {}
      this.downloadTempPath = null
    }

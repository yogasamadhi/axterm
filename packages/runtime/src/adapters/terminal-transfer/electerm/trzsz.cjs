...
function configureTrzszTransfer (implementation) {
  TrzszTransfer = implementation
}
...
class FileReader {
  constructor (filePath, fileName) {
    ...
    this.filePath = `${filePath}.axterm-${generate()}.part`
...
class FileWriter {
  ...
  async deleteFile () {
    ...
    if (!this.committed) await fs.promises.unlink(this.filePath).catch(() => {})
  ...
  async closeFile () {
    ...
    if (!this.committed) {
      ...
      await fs.promises.rename(this.filePath, this.finalPath)
      this.committed = true
    }
  }
}
...
class TrzszSession {
  ...
  endSession () {
    ...
    for (const writer of this.fileWriters) {
      try { writer.deleteFile() } catch (e) { log.error('Error deleting incomplete file', e) }
    }
...
module.exports = { configureTrzszTransfer, TrzszManager, TrzszSession, trzszManager, TRZSZ_STATE }

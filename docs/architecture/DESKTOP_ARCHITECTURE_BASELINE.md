...
> Axterm 当前固定采用 Level 1。本项目的产品目标、Electerm 5.5.0 复刻范围和执行顺序
> 以 [MASTER_SPEC](./MASTER_SPEC.md) 与
> [ELECTERM_PARITY_ROADMAP](../implementation/ELECTERM_PARITY_ROADMAP.md) 为准。
...
Core Runtime 是独立服务进程，使用 Node API，通过 Adapter 收敛执行环境差异。桌面分发由 Electron 内嵌 Node 承载，不要求用户另装系统 Node；Headless/Server 则使用独立 Node。它必须：
...
Axterm 当前使用 Bun 1.4.x 管理依赖和构建，Headless Core Runtime 与 Runtime 测试使用
Node 24.18+ 的 24.x 版本线，Desktop Core Runtime 由 Electron `utilityProcess` 承载并使用
Electron 内嵌 Node。精确解析版本以 `docs/implementation/VERSIONS.md` 为准，版本选择不由
Level 数字决定。

```text
Headless / Server = Node 24.18+ (24.x)
Desktop Runtime   = Electron embedded Node
Build / install   = Bun 1.4.x
```

Runtime-specific API 必须收敛在 Adapter 中。Axterm 的 Runtime 保留独立 Node Headless 入口，
业务模块不自行判断执行环境，也不得依赖 `bun:*` runtime-only API。

插件框架不是三个 Level 的共同前置条件。Axterm 当前 Level 1 使用普通模块和显式依赖注入，
不使用 Plugin Kernel。发布产物必须包含完整 Runtime 模块、配置和依赖，不依赖源码目录或
外部 workspace 链接。
...
资源回收顺序由真实依赖决定，不要求所有独立资源全局串行逆序释放。Level 1 使用显式
cleanup owner；独立资源可以并发清理，有先后依赖的资源必须由同一个 owner 按顺序等待释放。

...
> Axterm 当前使用本 Level；具体进程边界、产品能力和 Electerm 复刻路线以
> [MASTER_SPEC](./MASTER_SPEC.md) 与
...
本文是 Axterm 当前采用的架构等级。Phase 11–21 的 Electerm 复刻继续使用普通模块、
Application interface、Adapter 和显式 cleanup owner；功能范围扩大不自动触发架构升级。
...
有顺序依赖的资源必须明确等待释放完成；互不依赖的资源可以并发回收。开发时可通过 supervisor 重启整个 Runtime，无需为模块引入 Cordis Fiber 或插件 HMR。
...
真正升级时必须先写新的 ADR，说明触发信号、迁移边界、生命周期和回滚方案。在 ADR 通过
之前，Phase 11–21 不得引入 Internal Plugin Kernel、Bundle 或 Profile 装配。
